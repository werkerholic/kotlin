...
class PreprocessedFileCreator(val project: Project) {
    ...
    fun create(file: VirtualFile): VirtualFile = validExts.mapFirstNotNull { it.createPreprocessedFile(file) } ?: file
...
// extracted as possible candidates to stdlib
private inline fun<T: Any?, R> Array<T>.mapFind(mapper: (T) -> R?, predicate: (R?) -> Boolean): R? {
    for (element in this) {
        val res = mapper(element)
        if (predicate(res)) return res
    }
    return null
}

private inline fun<T: Any?, R> Array<T>.mapFirstNotNull(mapper: (T) -> R?): R? = mapFind(mapper, { it != null })
