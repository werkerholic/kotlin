...
private fun filterStringBuilderContents(content: StringBuilder, sectionIds: Collection<String>): StringBuilder {
    ...
    var curPos = 0
    val sectionsEnum = FilteredSectionsTokensRangeEnumerator(content, sectionIds)
    while (sectionsEnum.hasMoreElements()) {
        val range = sectionsEnum.nextElement()
...
private class FilteredSectionsTokensRangeEnumerator(script: CharSequence, val sectionIds: Collection<String>) : Enumeration<TokenRange> {
    ...
    override fun hasMoreElements(): Boolean = currentRange != null

    override fun nextElement(): TokenRange {
